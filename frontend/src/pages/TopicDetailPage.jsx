...
import { getTopicDetails, getQuestions } from "../api.js";
...
export default function TopicDetailPage() {
  ...
  return (
    <main style={{ maxWidth: 900, margin: "0 auto", padding: "0 24px" }}>
      <section style={{ paddingTop: 40, paddingBottom: 32 }}>
        <button onClick={() => navigate("/topics")} style={{ fontFamily: "var(--font-dm-sans)", fontSize: 13, color: "var(--muted-foreground)", background: "none", border: "none", cursor: "pointer", padding: 0, marginBottom: 20, display: "block" }}>
          ← Topics
        </button>
        ...
        <div style={{ display: "flex", gap: 28, flexWrap: "wrap", marginBottom: 28 }}>
          {[{ label: "Easy", count: topic.difficulty?.easy || 0 }, { label: "Medium", count: topic.difficulty?.medium || 0 }, { label: "Hard", count: topic.difficulty?.hard || 0 }].map(({ label, count }) => (
